...
class BasePage:
    ...
    def wait_visibility_ele(self, loc, loc_desc='', timeout=20, poll_time=0.5):
        ...
        """
        try:
            ele = WebDriverWait(self.driver, timeout=timeout, poll_frequency=poll_time).until(
    ...
    def wait_presence_ele(self, loc, loc_desc='', timeout=20, poll_time=0.5):
        ...
        try:
            ...
            log.error('等待元素-【{}】-加载到页面超时'.format(loc_desc))
            log.exception(e)
            self.page_save_screenshot(loc_desc)
            raise e
        else:
            log.info('等待元素-【{}】-加载到页面可见成功'.format(loc_desc))
            return ele
